...
public class BookServiceImpl implements BookService {
    ...
    public String saveUpdateBook(BookDto bookDto) {
        ...
        //Update
        Book book=bookRepo.findById(bookDto.getId()).orElseThrow(()->new CustomException("Book Not Found", HttpStatus.NOT_FOUND));
        CompletableFuture<Set<Author>> authorFuture = CompletableFuture.supplyAsync(() -> {
            return saveUpdateAuthor(bookDto);
        }).exceptionally(ex -> {
            throw new CustomException("Error while updating author: " + ex.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        });
        CompletableFuture<List<Genre>> genreFuture = CompletableFuture.supplyAsync(() -> {
            return saveUpdateGenre(bookDto);
        }).exceptionally(ex -> {
            throw new CustomException("Error while updating genre: " + ex.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        });
        try {
            Set<Author> authors = authorFuture.get();
            List<Genre> genres = genreFuture.get();
    ...
    @Override
    public PagewiseRes<BookDto> getAllBooksPagewise(Integer pageNumber, Integer pageSize, String sortBy, String sortDirection) {
        Sort sort;
    ...
    public BookDto getBookByISBN(String isbn) {
        return bookMapper.entityToDto(bookRepo.findByISBN(isbn).orElseThrow(()->new CustomException("Book Not found",HttpStatus.NOT_FOUND)));
    }
